...
foreach ($result as $node) {
    ...
    if ($slash = strpos($url, '/')) {
        ...
        $directory = '';
        $filename = $url . '.md'; // Use the title directly as the filename
    }
    ...
    $pandoc = new Pandoc\Pandoc('C:\Users\hp\Desktop\pandoc-3.1.11.1\pandoc.exe');
    $options = array(
        "from"  => "mediawiki",
        "to"    => $format
    );
    ...
    if (!empty($directory)) {
        if (!file_exists($directory)) {
            mkdir($directory, 0777, true); // The third parameter 'true' creates nested directories if needed
        }
    ...
    // create file
    $file = fopen(normalizePath($directory . $filename), 'w');
    fwrite($file, $text);
    fclose($file);
